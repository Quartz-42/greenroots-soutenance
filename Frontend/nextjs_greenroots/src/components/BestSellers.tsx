...
export default function BestSellers() {
  ...
  const { data: product1, loading: loading1, error: error1 } = 
    useFetch<Product>(randomIds[0] ? `products/${randomIds[0]}` : "");
  const { data: product2, loading: loading2, error: error2 } = 
    useFetch<Product>(randomIds[1] ? `products/${randomIds[1]}` : "");
  const { data: product3, loading: loading3, error: error3 } = 
    useFetch<Product>(randomIds[2] ? `products/${randomIds[2]}` : "");
  const { data: product4, loading: loading4, error: error4 } = 
  ...
  const isLoading = loading1 || loading2 || loading3 || loading4 || randomIds.length === 0;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {bestSellerProducts && bestSellerProducts.map((product) => (
            <ProductCard
              ...
              price={product.price}
              imageUrl={product.Image && product.Image[0] ? product.Image[0].url : "/product.png"}
            />
          ))}
        </div>
      </div>
